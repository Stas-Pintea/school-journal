...
export const upsertGrade = async (req, res) => {
  ...
  const parsedLessonNo = Number.isFinite(Number(lessonNo)) && Number(lessonNo) >= 1
    ? Math.floor(Number(lessonNo))
    : 1;
  ...
  const dateFilter = {
    ...
    date,
    ...(parsedLessonNo === 1
      ? { $or: [{ lessonNo: 1 }, { lessonNo: { $exists: false } }, { lessonNo: null }] }
  ...
  const update = {
    ...
    lessonNo: isDateKind ? parsedLessonNo : null,
...
export const deleteGradeByKey = async (req, res) => {
  const { student, class: classId, subject, kind, date = null, lessonNo = 1, period = null, task = null } = req.body;
